...
String & String::copy(const char *cstr, unsigned int length_ ) {
    if(!reserve(length_)) {
        invalidate();
        return *this;
    }
...
#ifdef FLASHSTRING
String & String::copy(const __FlashStringHelper *pstr, unsigned int length_) {
    if (!reserve(length_)) {
        invalidate();
        return *this;
    }
...
unsigned char String::concat(const char *cstr, unsigned int length_) {
    unsigned int newlen = len + length_;
    if(!cstr)
        return 0;
    if(length_ == 0)
        return 1;
...
void String::replace(char find, char replace_) {
    if(!buffer)
        return;
...
void String::replace(const String& find, const String& replace_) {
    if(len == 0 || find.len == 0)
        return;
    int diff = replace_.len - find.len;
    char *readFrom = buffer;
    char *foundAt;
    if(diff == 0) {
        while((foundAt = strstr(readFrom, find.buffer)) != NULL) {
            memcpy(foundAt, replace_.buffer, replace_.len);
            readFrom = foundAt + replace_.len;
        }
    } else if(diff < 0) {
        char *writeTo = buffer;
        while((foundAt = strstr(readFrom, find.buffer)) != NULL) {
            ...
            writeTo += n;
            memcpy(writeTo, replace_.buffer, replace_.len);
            writeTo += replace_.len;
        ...
        while(index >= 0 && (index = lastIndexOf(find, index)) >= 0) {
            ...
            len += diff;
            buffer[len] = 0;
            memcpy(buffer + index, replace_.buffer, replace_.len);
            index--;
        }
    }
}
...
void String::trim(void) {
    if(!buffer || len == 0)
        return;
    char *begin_ = buffer;
    while(isspace(*begin_))
        begin_++;
    char *end_ = buffer + len - 1;
    while(isspace(*end_) && end_ >= begin_)
        end_--;
    len = end_ + 1 - begin_;
    if(begin_ > buffer)
        memcpy(buffer, begin_, len);
    buffer[len] = 0;
}
